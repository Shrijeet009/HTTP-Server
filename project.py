...
def authentication_del(connectionSocket,date,time):
...
def delete_file(delete_name,date,time):
...
#	print(op)
	...
	if(words[0] == "GET"):
		...
		handle_post_request(date, time, connectionSocket, addr,filename)
	elif(words[0] == "PUT"):
...
def handle_post_request(date, time, connectionSocket, addr,filename):
	post2=connectionSocket.recv(1024).decode()
	print(post2)
	if(post2):
		f = open("success.html")
		response = f.read()
...
def handle_delete_request(date, time, connectionSocket, addr,words):
	...
	authenticate_val = authentication_del(connectionSocket,date,time)
	if(authenticate_val == 1):
		delete_name=name[1]
		del_file = delete_file(delete_name,date,time)
		if(del_file):
			string = "HTTP/1.1 200 OK \n"
			string += "DATE AND TIME: {} {}\n".format(date,time)			
